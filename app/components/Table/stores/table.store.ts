...
const [
  ...
  const queryBuilder = ref<IQueryBuilderRow[]>(tableProps?.queryBuilder ?? state.value.queryBuilder ?? [])
  ...
  const totalRows = ref(tableProps?.rows?.length ?? 0)
    ...
    const res = await fn(
      () => loadMetaData.value?.fnc?.({ tablePayload: fetchPayload, getStore }),
      {
        ...
        onError: ({ error, response }) => {
          ...
          loadMetaData.value?.onError?.(error, response, getStore)
        },
      },
    )
...
export function useTableStore(payload?: IConfig) {
  return useConsumeTableStore() ?? useProvideTableStore(payload)
}
