...
<template>
  <Dialog
    ...
    :ui="{ contentClass: ({ defaults }) => `${defaults.all} !p-x-3 !p-t-3 !p-b-1` }"
    @before-show="loadLayout"
    @hide="handleHide"
  >
    <Form
      :ui="{
        contentClass: ({ defaults }) => 'grid md:grid-cols-2 gap-2 overflow-auto',
        controlsClass: ({ defaults }) => `${defaults.all} !p-x-0 !p-b-0 m-t-3`,
      }"
