...
// Constants
import { BOOLEANISH_COMPARATORS, NON_VALUE_COMPARATORS } from '#layers/utilities/shared/constants/comparators-by-category.const'
...
const hasUnusedComparator = computed(() => {
  ...
  const columnComparators = columnCopy.value.filters.flatMap(filter => {
    const isBooleanishComparator = BOOLEANISH_COMPARATORS.includes(filter.comparator)
