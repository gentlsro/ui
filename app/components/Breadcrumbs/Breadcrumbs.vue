<script setup lang="ts">
import { uiConfig } from '$uiConfig'

// Types
import type { IBreadcrumbsProps } from './types/breadcrumbs-props.type'

// Functions
import { useBreadcrumbs } from './functions/useBreadcrumbs'

// Constants
import { BUTTON_PRESET } from '../Button/constants/button-preset.constant'
import { BREADCRUMBS_DEFAULT_PROPS } from './constants/breadcrumbs-default-props.constant'

const props = withDefaults(defineProps<IBreadcrumbsProps>(), {
  ...getComponentProps('breadcrumbs'),
})

// Utils
const { breadcrumbs } = useBreadcrumbs()

const mergedProps = computed(() => {
  return getComponentMergedProps('breadcrumbs', props)
})

// Styles - wrapper
const containerClass = computed(() => {
  return mergedProps.value?.ui?.containerClass?.({
    defaults: BREADCRUMBS_DEFAULT_PROPS.ui.containerClass(),
  })
})

const containerStyle = computed(() => {
  return mergedProps.value?.ui?.containerStyle?.()
})

// Styles - breadcrumbs
const breadcrumbsClass = computed(() => {
  return mergedProps.value?.ui?.breadcrumbsClass?.({
    defaults: BREADCRUMBS_DEFAULT_PROPS.ui.breadcrumbsClass(),
  })
})

const breadcrumbsStyle = computed(() => {
  return mergedProps.value?.ui?.breadcrumbsStyle?.()
})

// Layout
const breadcrumbsItems = computed(() => {
  const homePath = typeof uiConfig.breadcrumbs.home.path === 'function'
    ? uiConfig.breadcrumbs.home.path()
    : uiConfig.breadcrumbs.home.path

  const homeBreacrumb = uiConfig.breadcrumbs.home.component
    ? { component: uiConfig.breadcrumbs.home.component }
    : {
        to: homePath,
        icon: uiConfig.breadcrumbs.home.icon,
        label: uiConfig.breadcrumbs.home.label,
      }

  const usedBreadcrumbs = props.breadcrumbs ?? breadcrumbs.value

  return [
    homeBreacrumb,
    ...usedBreadcrumbs,
  ]
    .flatMap(breadcrumb => [breadcrumb, 'splitter'])
    .slice(0, -1)
})
</script>

<template>
  <div
    class="breadcrumbs-wrapper group/breadcrumbs"
    :class="containerClass"
    :style="containerStyle"
  >
    <div
      v-if="breadcrumbsItems.length > 1"
      class="breadcrumbs"
      :class="breadcrumbsClass"
      :style="breadcrumbsStyle"
    >
      <HorizontalScroller
        :ui="{ contentClass: ({ defaults }) => `${defaults.all} gap-1 items-center` }"
      >
        <template
          v-for="(breadcrumb, idx) in breadcrumbsItems"
          :key="idx"
        >
          <!-- Chevron -->
          <span
            v-if="typeof breadcrumb === 'string'"
            :class="BUTTON_PRESET.CHEVRON_RIGHT.icon"
            class="breadcrumb-item shrink-0"
          />

          <!-- Component -->
          <Component
            :is="breadcrumb.component"
            v-else-if="'component' in breadcrumb && breadcrumb.component"
          />

          <!-- Button -->
          <Btn
            v-else
            v-bind="breadcrumb"
            no-active-link
            color="!dark !dark:light"
            size="sm"
            class="breadcrumb-item shrink-0"
            no-truncate
            :class="[{ 'is-last': breadcrumb === breadcrumbs[breadcrumbs.length - 1] }]"
            :no-dim="breadcrumb === breadcrumbs[breadcrumbs.length - 1]"
            no-uppercase
          />
        </template>
      </HorizontalScroller>

      <slot name="append" />
    </div>

    <slot name="right" />
  </div>
</template>

<style lang="scss">
// No styles needed - all moved to default-props constant
</style>
